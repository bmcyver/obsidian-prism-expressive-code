...
export function setLuminance(input: string, targetLuminance: number) {
  return withParsedColor(input, (color) => {
    ...
    const mixAmount = binarySearch({
      ...
      preferHigher:
        targetLuminance > 0 && targetLuminance < 1 ? increasing : undefined,
      tolerance: 1 / 256,
...
export function getColorContrastOnBackground(
  input: string,
  background: string,
) {
...
export function changeLuminanceToReachColorContrast(
  input1: string,
  input2: string,
  minContrast = 6,
): string {
...
export function changeAlphaToReachColorContrast(
  input: string,
  background: string,
  minContrast = 6,
  maxContrast = 22,
 ...
 */
export function chromaticRecolor(
  input: string,
  target: string | ChromaticRecolorTarget,
) {
  ...
  return withParsedColor(input, (color) => {
    ...
    const linearDecrease = (i: number, start: number, end: number) =>
...
function withParsedColor(
  input: string,
  transform: (color: TinyColor) => string,
  fallback?: string,
) {
