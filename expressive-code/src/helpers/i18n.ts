...
export function formatTemplate(
  ...
  const getReplacement = (varName: string, ...choices: string[]) => {
    ...
    if (value === undefined)
      throw new Error(
        `Unknown variable name "${varName}" found in string template "${template}". Available variables: ${JSON.stringify(Object.keys(variables))}`,
      );
    if (!choices.length) return value.toString();
    const parsedChoices: {
      condition?: { operator: string; number: number } | undefined;
      text: string;
    }[] = choices.map((choice) => {
    ...
    const catchAllCount = parsedChoices.filter(
      (choice) => !choice.condition,
    ).length;
    if (catchAllCount !== 1)
      throw new Error(
        `Expected exactly 1 catch-all choice for variable "${varName}", but found ${catchAllCount} in string template "${template}".`,
      );
