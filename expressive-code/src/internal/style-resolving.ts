...
export function resolveStyleSettings({
  ...
  const preventUnitlessValues = new Set<StyleSettingPath>(
    coreStyleSettings.preventUnitlessValues,
  );
  plugins.forEach((plugin) => {
    plugin.styleSettings?.preventUnitlessValues.forEach((path) =>
      preventUnitlessValues.add(path),
    );
  });
  ...
  function resolveSetting(settingPath: StyleSettingPath): string {
    ...
    if (result === undefined && !resolvedByPath.has(settingPath)) {
      if (attemptedToResolve.has(settingPath))
        throw new Error(
          `Circular dependency detected while resolving style setting '${settingPath as string}'`,
        );
...
type UnresolvedStyleSettingsByPath = Map<
  StyleSettingPath,
  UnresolvedStyleValue
>;
...
function getStyleSettingsByPath(styleSettings: {
  [K: string]: UnresolvedStyleValue | { [K: string]: UnresolvedStyleValue };
}): UnresolvedStyleSettingsByPath {
...
function applyStyleSettings(
  target: UnresolvedStyleSettingsByPath,
  source: UnresolvedStyleSettingsByPath,
) {
  source.forEach(
    (value, path) => value !== undefined && target.set(path, value),
  );
}
