...
export class Cm6_SyntaxTreeParser {
	...
	static getExpandedViewportRange(view: EditorView): { from: number; to: number } {
	...
	static getDecorationUpdates(view: EditorView, plugin: ShikiPlugin, docChanged: boolean): DecorationUpdate[] {
		let lang = '';
			...
			enter: nodeRef => {
				...
				if (props.has('inline-code')) {
					...
					if (content.endsWith('}') && plugin.settings.inlineHighlighting) {
						...
						decorationUpdates.push({ type: DecorationUpdateType.Remove, from: node.from, to: node.to });
					}
					return;
				}
