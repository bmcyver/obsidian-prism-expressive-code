import { flattenTokens, getStyleForPrismTypes, LANGUAGE_ALIASES, FontStyle, type FlatToken, type ThemeLike } from 'src/prism/PrismUtils';
...
export class InlineHighlighter {
	...
	public async getHighlightTokens(code: string, lang: string, prism: typeof Prism, safeLanguagesSet: Set<string>, supportedLanguages: string[]): Promise<TokensResult | undefined> {
		...
		const themedTokens = this.convertToThemedTokens(flatTokens, theme as ThemeLike, lowerLang);
	...
	private convertToThemedTokens(flatTokens: FlatToken[], theme: ThemeLike, lang?: string): ThemedToken[] {
